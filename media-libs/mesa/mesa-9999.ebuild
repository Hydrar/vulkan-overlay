...
DEPEND="${RDEPEND}
	...
	vulkan? (
		>=sys-devel/llvm-3.9.0_rc3:=[${MULTILIB_USEDEP}]
		>=sys-devel/clang-3.9.0_rc3-r100:=[${MULTILIB_USEDEP}]
	)
"
[[ ${PV} == 9999 ]] && DEPEND+="
	sys-devel/bison
...
multilib_src_configure() {
	...
	if use vulkan; then
		...
		myconf+="--with-vulkan-drivers=${vulkan_drivers}"
	fi
	...
	econf \
		...
		PYTHON2="${PYTHON}" \
		${myconf}
		CFLAGS="-Wno-implicit-function-declaration -Wno-missing-prototypes -Wno-unused-label"
}
...
multilib_src_install() {
	...
	# if use vulkan; then
	# 	ebegin "Moving Vulkan implementation for dynamic switching"
	# 	local vk_dir="/usr/$(get_libdir)/vulkan/vendors/mesa"
	# 	dodir ${vk_dir}/{lib,include}
	# 	dodir /etc/vulkan/icd.d
	# 	# dosym ../../../../etc/vulkan/icd.d/radeon_icd.json /usr/share/vulkan/icd.d/radeon_icd.json
	# 	dosym ../../../usr/share/vulkan/icd.d/radeon_icd.json /etc/vulkan/icd.d/radeon_icd.json
	# 	eend $?
	# fi
